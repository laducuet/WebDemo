...
function getChildCategories($db)
{
    $sql = "call GetAllChildCategories();";
    $stmt = $db->query($sql);
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
}
...
function getItemsByChildCategory($db, $childcategoryId)
{
    $sql = "SELECT * FROM item WHERE item.childcategoryid = $childcategoryId";
    $stmt = $db->query($sql);
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
}

function getChildByCategory($db, $categoryId)
{
    $sql = "SELECT * FROM childcategory WHERE childcategory.categoryid = $categoryId";
    $stmt = $db->query($sql);
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
}
...
function insertItem($title,$Des,$info,$price,$quantity,$childcatId,$discount,$sellerid,$homeNum,$street,$city,$country,$db){
    ...
    function updateTotalItems($childcatId, $quantity, $db) {
        $sql = "UPDATE childcategory SET totalItems = totalItems + :quantity WHERE childcategoryId = :childcatId";
        $stmt = $db->prepare($sql);
    ...
    function getItemQuantity($itemId, $db) {
    ...
    function getChildCategoryId($itemId, $db) {
        $sql = "SELECT childcategoryId FROM item WHERE itemId = :itemId";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(":itemId", $itemId, PDO::PARAM_INT);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result["childcategoryId"];
    }

    function deleteTotalItems($childcatId, $quantity, $itemId, $db) {
        $quantity_item = getItemQuantity($itemId, $db);
        $sql = "UPDATE childcategory SET totalItems = totalItems - $quantity_item + :quantity WHERE childcategoryId = :childcatId";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':quantity', $quantity, PDO::PARAM_INT);
    ...
    function retriveTotalItems($childcatId, $quantity, $itemId, $db) {
        $sql = "UPDATE childcategory SET totalItems = totalItems + :quantity WHERE childcategoryId = :childcatId";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':quantity', $quantity, PDO::PARAM_INT);
        $stmt->bindParam(':childcatId', $childcatId, PDO::PARAM_INT);
    ...
    function insertImageCategoris($Arrimage,$db){
        $itemID=$db->lastInsertId();
        foreach($Arrimage as $image){
            $sql="INSERT INTO category (itemId,image) 
            VALUES (".$itemID.",'".$image."')";
            $stmt=$db->prepare($sql);
            $stmt->execute();
        }
...
function getChildCategory($childcatId, $db)
{
    $sql = "SELECT * from childcategory WHERE childcategory.childcategoryId = :childcatId";
    $stmt = $db->prepare($sql);
    $stmt->execute(array(":childcatId" => $childcatId));
    $rows = $stmt->fetchAll(PDO::FETCH_CLASS);
    return $rows;
}
...
function updateChildCategory($db,$itemID,$childcat){
    $sql="UPDATE `item` SET `childcategoryId` = '".$childcat."' WHERE `item`.`itemId` = ".$itemID."";
    $stmt = $db->prepare($sql);
    $stmt->execute();
}
...
function getChildCategoryName($childcatId, $db)
{
    $sql = "SELECT * from childcategory WHERE childcategory.childcategoryId = ".$childcatId."";
    $stmt = $db->query($sql);
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
function setisSaled($id,$db){
    $sql = "UPDATE item SET item.isSaled = 1 WHERE item.itemId = :id";
    $stmt = $db->prepare($sql);
    $stmt->execute(array(":id" => $id));
}
...
function getQuantityFromOrdersAndItems($itemId, $db) {
    $sql = "SELECT SUM(orders.quantity) as totalQuantity FROM orders 
            JOIN item ON orders.itemId = item.itemId
            WHERE orders.itemId = :itemId AND orders.status = 1
